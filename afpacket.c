...
static int __get_interface_by_device_name(int socket_fd, const char *name_of_device) {
    struct ifreq ifr;
...
static struct iovec* __setup_rx_ring(int socket_fd, struct sockaddr_ll *bind_address) {
    ...
    if ((mapped_buffer = (uint8_t*)mmap(NULL, req.tp_block_size * req.tp_block_nr, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_LOCKED, socket_fd, 0)) == MAP_FAILED){
        return NULL;
    }
...
static void __process_block(struct block_desc *pbd, const int block_num, packet_handler callback, uint8_t *user_data) {
    int num_pkts = pbd->header.num_pkts, i;
    struct tpacket3_hdr *ppd;

    ppd = (struct tpacket3_hdr *) ((uint8_t *) pbd +
                       pbd->header.offset_to_first_pkt);
