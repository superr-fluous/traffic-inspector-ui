...
void free_workflow(ndpi_workflow_t ** const workflow) {
  struct ndpi_workflow_t * const w = *workflow;
  size_t i;
...
static int __ip_tuples_compare(struct nDPI_flow_info const * const A, struct nDPI_flow_info const * const B)
{
...
static void __ndpi_idle_scan_walker(void const * const A, ndpi_VISIT which, void * const user_data)
{
...
static void __check_for_idle_flows(ndpi_workflow_t * const workflow)
{
  ...
  if (workflow->last_idle_scan_time + IDLE_SCAN_PERIOD < workflow->last_time) {
	...
	ndpi_flow_info_t * const f =
	  (ndpi_flow_info_t *)workflow->ndpi_flows_idle[--workflow->cur_idle_flows];
...
{
  ndpi_work_thread_t * const reader_thread =
    (ndpi_work_thread_t *)args;
  struct nDPI_workflow * workflow;
  struct nDPI_flow_info flow;

  size_t hashed_index;
  void * tree_result;
  struct ndpi_flow_info_t * flow_to_process;
  ...
  if (ndpi_is_protocol_detected(flow_to_process->detected_l7_protocol) != 0 &&
      ...
      flow_to_process->ndpi_flow->max_extra_packets_to_check)
    {
      ...
      ndpi_dpi2json(workflow->ndpi_struct, flow_to_process->ndpi_flow, flow_to_process->detected_l7_protocol, &workflow->json_serializer);
